...
Query `snowplow_identities_identifier_mapping` instead of this table unless you want the raw event-time record. The view resolves each row to its current `active_snowplow_id`.
{% enddocs %}
